...
public class AccionesPlanPago {
	...
	public static PlanPagos getAllPlan(int dni) throws Exception{
		
		PlanPagos planes = new PlanPagos();
		PlanPago p;
				
		Statement stm = Conexion.conectar().createStatement();						
		ResultSet rs = stm.executeQuery("SELECT * FROM PLAN_CUOTAS WHERE DNI=" + dni);
	...
	public static PlanPago getOnePlan(int dni, String fecha, int periodoini, int periodofin) throws Exception{
		
		PlanPago p = null;
				
		Statement stm = Conexion.conectar().createStatement();			
		
		//System.out.println("SELECT COD_PLAN, DNI, AÑOINI, AÑOFIN, PERIODOINI, PERIODOFIN, FECHA_ALTA , IFNULL(OBSERVACIONES, '') AS OBSERVACIONES FROM PLAN_CUOTAS WHERE dni = " + dni + " AND FECHA_ALTA = '" + fecha + "' AND PERIODOINI = " + periodoini + " AND PERIODOFIN = " + periodofin);
		
		ResultSet rs = stm.executeQuery("SELECT COD_PLAN, DNI, AÑOINI, AÑOFIN, PERIODOINI, PERIODOFIN, FECHA_ALTA , IFNULL(OBSERVACIONES, '') AS OBSERVACIONES FROM PLAN_CUOTAS WHERE dni = " + dni + " AND FECHA_ALTA = '" + fecha + " AND PERIODOINI = " + periodoini + "' AND PERIODOFIN = " + periodofin);
	...
	public static int deletePagosPlanPagoDni(int dni) throws Exception{
		
		int i = 0;
		
		Statement stmt = Conexion.conectar().createStatement();	
		
		stmt.executeUpdate("DELETE FROM PLAN_PAGOS WHERE DNI = " + dni);
	...
	public static int deletePlanPagoDni(int dni) throws Exception{
		
		int i = 0;
		
		Statement stmt = Conexion.conectar().createStatement();	
			
		stmt.executeUpdate("DELETE FROM PLAN_CUOTAS WHERE DNI = " + dni);
	...
	public static PagosPlanPago getPagosAll(int dni) throws Exception{
		
		PagosPlanPago pagospp = new PagosPlanPago();
				
		Statement stm = Conexion.conectar().createStatement();			
		ResultSet rs = stm.executeQuery("SELECT * FROM PLAN_PAGOS WHERE DNI = "+ dni);
	...
	public static void main(String[] args) {
		PlanPago p;
		try {
			p = AccionesPlanPago.getOnePlan(3, "2015-05-04", 4, 12);
			
			System.out.println("plan= " + p.getDni() + p.getFecha());
		} catch (Exception e) {
			e.printStackTrace();
		}
