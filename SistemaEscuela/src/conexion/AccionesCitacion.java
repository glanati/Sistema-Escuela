...
public class AccionesCitacion {
	...
	public static Citaciones getAll(int dni) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT * FROM CITACIONES WHERE DNI =" + dni);			
			Citacion tmp;
			
			while (rs.next()) {
				tmp = new Citacion (rs.getInt("DNI"),rs.getString("FECHA"), rs.getString("HORA"), rs.getString("DESCRIP"));
	...
	public static int deleteAllDni(int dni) {
		...
		try {
			Statement stmt = Conexion.conectar().createStatement();
			i = stmt.executeUpdate("DELETE FROM CITACIONES WHERE DNI = " + dni);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return i;
