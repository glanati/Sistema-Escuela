...
public class AccionesCertificado {
	...
	public static Certificados getAllDni(int dni) {
		Certificados lista = new Certificados();
		try {
			Statement stmt = Conexion.conectar().createStatement();
			ResultSet rs = stmt.executeQuery("SELECT * FROM CERTIFICADOS WHERE DNI=" + dni);
			Certificado tmp;
...
//				i = 1;
	...
	public static void insertOne(Certificado c) throws SQLException, Exception { //inserta los valores por defecto (0) de los indicadores de certificados
		Statement smt1 = Conexion.conectar().createStatement();
				
		smt1.executeUpdate("INSERT INTO CERTIFICADOS (DNI, AÑO, IND_SALUD, IND_BUCAL, IND_DNI, IND_AUDITIVO, IND_VISUAL, IND_VACUNAS) VALUES (" + c.getDni() + "," + c.getAño() + "," + boolToByte(c.isInd_salud()) + "," + boolToByte(c.isInd_bucal()) +  "," + boolToByte(c.isInd_dni()) +  "," + boolToByte(c.isInd_auditivo()) +  "," + boolToByte(c.isInd_visual()) +  "," + boolToByte(c.isInd_vacunas()) + " )");
	...
	public static void insertOne(int dni, int año) throws SQLException, Exception { //inserta los valores por defecto (0) de los indicadores de certificados
		...
		smt1.executeUpdate("INSERT INTO CERTIFICADOS (DNI, AÑO) VALUES (" + dni + "," + año + " )");
		
		smt1.close();
		Conexion.desconectar();
	}	
	
	public static int deleteAllDni(int dni) {
		int i = 0;
		try {
			Statement stmt = Conexion.conectar().createStatement();
