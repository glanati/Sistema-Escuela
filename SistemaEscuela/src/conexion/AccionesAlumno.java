...
public class AccionesAlumno {
	...
	public static Alumnos_Grados getAlumno_Grado(int dni) throws Exception{
		Alumnos_Grados alumnos = new Alumnos_Grados();
		Alumno_Grado a;
		try{
			Statement stm = Conexion.conectar().createStatement();
			ResultSet rs = stm.executeQuery("SELECT * FROM ALUMNOS_GRADO WHERE DNI=" + dni);
			while(rs.next()){
				a = new Alumno_Grado(rs.getString("GRADO"),rs.getString("TURNO"),rs.getInt("DNI"),rs.getInt("AÑO"));
				alumnos.agregarAlumno_Grado(a);
			}
	...
	public static Subsidios getSubsidios(int dni) throws SQLException, Exception {
		Statement stm = Conexion.conectar().createStatement();		
		ResultSet rs = stm.executeQuery("SELECT * FROM ALUMNOS_SUBSIDIO WHERE DNI= " + dni);
		
		Subsidios subsidios = new Subsidios();
		Subsidio s;
		
		while(rs.next()){
			s = new Subsidio(rs.getInt("año"),rs.getInt("dni"),rs.getBoolean("ind_grupo"), rs.getBoolean("ind_subsidio"));
			subsidios.agregarSubsidio(s);
		}
	...
	public static void insertSubsidio(Subsidio s) throws SQLException, Exception {
		Statement stmt = Conexion.conectar().createStatement();			
			
		stmt.executeUpdate("INSERT INTO ALUMNOS_SUBSIDIO VALUES (" + s.getAño() + "," + s.getDni() + "," + boolToByte(s.isInd_grupo()) + "," + boolToByte(s.isInd_subsidio()) + ")");						
											
		stmt.close();
		Conexion.desconectar();
	}
	
	public static int deleteSubsidioDni(int dni) {
		int i = 0;
		try {
			Statement stmt = Conexion.conectar().createStatement();
			i = stmt.executeUpdate("DELETE FROM ALUMNOS_SUBSIDIO WHERE DNI = '" + dni + "'");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return i;
