...
public class AccionesEstado {
	...
	public static EstadoAlumnos getAll(int dni) throws SQLException, Exception {		// RECUPERA TODOS LOS ESTADOS DEL ALUMNO
		EstadoAlumnos a = new EstadoAlumnos();
		
			Statement stmt = Conexion.conectar().createStatement();
			ResultSet rs = stmt.executeQuery("SELECT DNI, FECHA, ACTIVO FROM ESTADO_ALUMNO WHERE DNI = " + dni);
			
			EstadoAlumno e;
			
			while (rs.next()) {				
				e = new EstadoAlumno(rs.getInt("dni"), rs.getString("fecha"), rs.getBoolean("activo"));				
				a.agregarEstadoAlumno(e);
			}
	...
	public static void insertEstado(EstadoAlumno e) throws SQLException, Exception {
		...
		Statement stmt = Conexion.conectar().createStatement();
		stmt.executeUpdate("INSERT INTO ESTADO_ALUMNO (DNI, FECHA,ACTIVO) VALUES (" + e.getDni() + ",'" + e.getFecha()+ "'," + estado + ")");
