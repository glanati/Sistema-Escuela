...
public class AccionesCuota {
	...
	public static Cuotas getPagosAll(int dni) throws Exception{
		
		Cuotas cuotas = new Cuotas();
				
		Statement stm = Conexion.conectar().createStatement();	
		
		ResultSet rs = stm.executeQuery("SELECT * FROM PAGOS_CUOTA WHERE DNI =" + dni);
	...
	public static int deletePagosDni(int dni) throws Exception{
		
		int i = 0;
		
		Statement stmt = Conexion.conectar().createStatement();		
	
		stmt.executeUpdate("DELETE FROM PAGOS_CUOTA WHERE DNI = " + dni);
