...
public class AccionesSancion {
	...
	public static Sanciones getAll(int dni) {
		Sanciones lista = new Sanciones();
		try {
			Statement stmt = Conexion.conectar().createStatement();
			ResultSet rs = stmt.executeQuery("SELECT * FROM SANCIONES WHERE DNI = "+ dni);			
			Sancion tmp;
			
			while (rs.next()) {
				tmp = new Sancion(rs.getInt("DNI"), rs.getString("FECHA"), rs.getString("HORA"), rs.getString("MOTIVO"));
				lista.agregarSancion(tmp);
			}
			stmt.close();
			Conexion.desconectar();
	...
	public static int deleteAllDni(int dni) {
		...
		try {
			Statement stmt = Conexion.conectar().createStatement();
			i = stmt.executeUpdate("DELETE FROM SANCIONES WHERE DNI = " + dni);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return i;
