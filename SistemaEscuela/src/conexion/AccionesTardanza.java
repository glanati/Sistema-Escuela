...
public class AccionesTardanza {
	...
	public static Tardanzas getAllTardanzasDni(int dni) throws Exception{
		Tardanzas tardanzas = new Tardanzas();
		try{
			Statement stm = Conexion.conectar().createStatement();
						
			ResultSet rs = stm.executeQuery("SELECT * FROM TARDANZAS WHERE TIPO = 'T' AND DNI = "+ dni);
			Tardanza t;
			while(rs.next()){
	...
	public static int bajaTardanzaDni(int dni) throws Exception{
		int i=0;
		try{
			Statement stm = Conexion.conectar().createStatement();
			i=stm.executeUpdate("DELETE FROM TARDANZAS WHERE TIPO = 'T' AND DNI="+dni);
			stm.close();
			Conexion.desconectar();
		}catch(SQLException sqle){
	...
	public static Tardanzas getAsistenciasDni(int dni) throws Exception{
		Tardanzas asistencias = new Tardanzas();
		Tardanza a =null;
		try{
			Statement stm = Conexion.conectar().createStatement();
			ResultSet rs = stm.executeQuery("SELECT * FROM TARDANZAS WHERE TIPO = 'A' AND DNI="+dni);
			while(rs.next()){
				a = new Tardanza(rs.getInt("DNI"),rs.getString("FECHA"),rs.getString("OBSERVACIONES"),rs.getString("TIPO"),rs.getString("INDICADOR"));
				asistencias.agregarTardanza(a);
			}	
	...
	public static int bajaAsistencia(int dni) throws Exception{
		int i=0;
		try{
			Statement stm = Conexion.conectar().createStatement();
			i=stm.executeUpdate("DELETE FROM TARDANZAS WHERE TIPO = 'A' AND DNI=" + dni);
			stm.close();
			Conexion.desconectar();
		}catch(SQLException sqle){
			System.out.println("error sql!!!");
